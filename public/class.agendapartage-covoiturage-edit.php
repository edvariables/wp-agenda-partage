 ...
 */
class AgendaPartage_Covoiturage_Edit extends AgendaPartage_Post_Edit_Abstract {

	const post_type_class = 'AgendaPartage_Covoiturage';
	...
	public static function init_hooks() {
		...
		add_action( 'wp_ajax_'.AGDP_TAG.'_'.AGDP_COVOIT_SECRETCODE, array(__CLASS__, 'on_wp_ajax_post_code_secret_cb') );
		add_action( 'wp_ajax_nopriv_'.AGDP_TAG.'_'.AGDP_COVOIT_SECRETCODE, array(__CLASS__, 'on_wp_ajax_post_code_secret_cb') );
	}
 	/////////////
	...
	public static function submit_covoiturage_form($contact_form, &$abort, $submission){
		...
		if($error_message){
			...
			foreach($tax_terms as $tax_name => $tax_inputs){
				...
				if( $tax_name = 'cov_diffusion'){
					debug_log('$tax_inputs', $tax_name, $tax_inputs);
					$meta_key = 'connexion';
					foreach( $tax_inputs as $tax_id ){
