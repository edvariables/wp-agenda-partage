 ...
 * En Admin, le bloc d'édition du Content est masqué d'après la définition du Post type : le paramètre 'supports' qui ne contient pas 'editor', see Agdp_Admin_Covoiturage::init_PostType_Supports
 *
 * Voir aussi Agdp_Admin_Covoiturage
 */
class Agdp_Covoiturage_Post_type {
	...
	public static function register_post_type() {
		...
		$args = array(
			...
			'labels'                => $labels,
			'supports'              => array( 'title', 'thumbnail', 'revisions' ),//, 'author', 'editor' see Agdp_Admin_Covoiturage::init_PostType_Supports
		...
		register_post_type( Agdp_Covoiturage::post_type, $args );
	...
	public static function register_taxonomy_city() {
		...
		register_taxonomy( Agdp_Covoiturage::taxonomy_city, array( Agdp_Covoiturage::post_type ), $args );
	...
	public static function register_taxonomy_diffusion() {
		...
		register_taxonomy( Agdp_Covoiturage::taxonomy_diffusion, array( Agdp_Covoiturage::post_type ), $args );
	...
	public static function register_user_role(){
		...
		// );
		// add_role( Agdp_Covoiturage::post_type, __('Covoiturage', AGDP_TAG ),  $capabilities);
	}

	/**
	 * Retourne tous les termes
	 */
	public static function get_all_intentions($array_keys_field = 'term_id', $query_args = []){
		return [
			'1' => 'Propose',
			'2' => 'Cherche'
		];
	...
	public static function get_all_diffusions($array_keys_field = 'term_id', $query_args = []){
		return Agdp_Covoiturage::get_all_terms(Agdp_Covoiturage::taxonomy_diffusion, $array_keys_field, $query_args);
	}
