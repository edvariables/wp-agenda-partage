...
class Agdp_User {
		...
		if ( ! array_key_exists( $blog_id, get_blogs_of_user( $user->ID, false ) ) ) {
			...
			if( current_user_can( 'promote_user', $user->ID )  ){
				...
				if(is_wp_error($result)){
					Agdp_Admin::add_admin_notice( sprintf(__("L'utilisateur %s n'a pas accès à ce site web pour la raison suivante : %s", AGDP_TAG), $user->display_name, $result->get_error_message()), 'error');
				}
				else {
					Agdp_Admin::add_admin_notice( sprintf(__("Désormais, l'utilisateur %s a accès à ce site web en tant qu'organisateur d'évènements.", AGDP_TAG), $user->display_name), 'success');
				}
			}
			else{
				Agdp_Admin::add_admin_notice( sprintf(__("L'utilisateur %s n'a pas accès à ce site web et vous n'avez pas l'autorisation de le lui accorder. Contactez un administrateur de niveau supérieur.", AGDP_TAG), $user->display_name), 'warning');
			}
		}
		return $user;
	}
	...
	public static function new_password_link($user_id, $redirect_to = false){
		...
		|| $user_id == Agdp_User::get_blog_admin_id()
		)
			return;
		if( ! isset($user))
			$user = new WP_USER($user_id);
	...
	public static function send_welcome_email($user_id, $subject = false, $message = false, $return_html_result = false){
		...
		$subscriptions = [];
		foreach( Agdp_Newsletter::get_newsletters() as $newsletter){
			$subscription_period_name = Agdp_Newsletter::subscription_period_name(Agdp_Newsletter::get_subscription($email, $newsletter), $newsletter);
			if($subscription_period_name){
				$page = Agdp_Newsletter::get_content_source_page( $newsletter->ID );
				if( $page ) 
		...
		$url = get_permalink(Agdp::get_option('newsletter_subscribe_page_id'));
		...
		foreach( Agdp_Forum::get_forums() as $forum){
			if( ( $subscription_role = Agdp_Forum::get_user_subscription($forum, $email) )
			&& isset( Agdp_Forum::subscription_roles[$subscription_role] )
			&& ( $subscription_role !== 'banned' )
		){
				$subscription_role_name = Agdp_Forum::subscription_roles[$subscription_role];
