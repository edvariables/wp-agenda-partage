 ...
 */
class AgendaPartage_Covoiturages_Export {
	...
	public static function export_posts_txt($posts){
		...
		foreach($posts as $post){
			$txt[] = $post->post_title;
			$txt[] = str_repeat('-', 24);
			$txt[] = AgendaPartage_Covoiturage::get_covoiturage_dates_text( $post->ID );
			$txt[] = get_post_meta($post->ID, 'cov-localisation', true);
			if( $value = AgendaPartage_Covoiturage::get_covoiturage_cities($post->ID))
				$txt[] = implode(', ', $value);
	...
	public static function add_covoiturage_to_ZCiCal($post, $ical){
		...
		$uid = sprintf('%s[%d]@%s', AgendaPartage_Covoiturage::post_type, $post->ID, $parse['host']);
