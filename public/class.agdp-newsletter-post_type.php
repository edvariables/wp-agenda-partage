 ...
 */
class Agdp_Newsletter_Post_type {
	...
	public static function register_post_type() {
		...
		$args = array(
			...
			'labels'                => $labels,
			'supports'              => array( 'title', 'editor', 'revisions' ),//, 'author', 'editor' see Agdp_Admin_Newsletter::init_PostType_Supports
			'taxonomies'            => array(  ),
			'hierarchical'          => false,
			'public'                => true,
			'show_ui'               => true,
		...
		register_post_type( Agdp_Newsletter::post_type, $args );
	...
	public static function register_user_role(){
		return;
			...
			// );
			// add_role( Agdp_Newsletter::post_type, __('Lettre-info', AGDP_TAG ),  $capabilities);
	}
	...
	public static function register_taxonomy_period() {
		...
		register_taxonomy( Agdp_Newsletter::taxonomy_period, array( Agdp_Newsletter::post_type ), $args );
	...
	public static function init_taxonomy(){
		...
		$existings = [];
		foreach(get_terms(Agdp_Newsletter::taxonomy_period) as $existing){
		...
		foreach( array_diff($terms, $existings) as $new_slug => $new_name)
			wp_insert_term($new_name, Agdp_Newsletter::taxonomy_period
				, array(
