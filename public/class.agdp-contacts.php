 ...
 */
class Agdp_Contacts extends Agdp_Posts {

	const post_type = Agdp_Contact::post_type;
	const postid_argument = Agdp_Contact::postid_argument;
	const page_id_option = Agdp_Contact::posts_page_option;
	...
	/**
	 * Recherche des contacts d'une semaine
	 * $year_week : yyyy-ww
	 */
	...
	/**
	 * Recherche de toutes les semaines contenant des contacts mais aussi les semaines sans.
		...
		if(count($filters)){
			...
			if(count($query_tax_terms)){
				...
				foreach($query_tax_terms as $tax_name => $tax_data){
					...
					;
					if($tax_name === Agdp_Contact::taxonomy_city
					&& count($tax_data['IN'])){
						$sql .=   "\n INNER JOIN {$blog_prefix}postmeta as localisation"
									. "\n ON post.ID = localisation.post_id"
									. "\n AND localisation.meta_key = 'cont-localisation'" //TODO
						;
					}
				}
				//WHERE
				foreach($query_tax_terms as $tax_name => $tax_data){
					if(count($tax_data['IN'])){
						...
						;
						if($tax_name === Agdp_Contact::taxonomy_city){
							$terms_like = Agdp_Contact_Post_type::get_terms_like($tax_name, $tax_data['IN']);
	...
	/**
	* Rendu Html des contacts sous forme d'arborescence par semaine
	*
	...
	*/
	public static function get_list_html($content = '', $options = false){
		...
		//Semaines avec indicateurs du nombre de contacts, sans les périodiques.
		...
		$requested_id = array_key_exists(AGDP_ARG_CONTACTID, $_GET) ? $_GET[AGDP_ARG_CONTACTID] : false;
		$requested_month = false;
		if($requested_id){
			$date_debut = get_post_meta($requested_id, 'cont-date-debut', true);
			if($date_debut)
				$requested_month = substr($date_debut, 0, 7);
		}
		
		$html = sprintf('<div class="agdp-contacts agdp-contacts-%s">', $options['mode']);
		if( $options['mode'] != 'email')
		...
		foreach($weeks as $week => $week_events_count) {
			...
			&& $week !== $requested_month) {
				...
				$ajax = sprintf('ajax="once" data="%s"',
					esc_attr( json_encode ( array(
						'action' => Agdp_Contact::post_type.'_show_more',
						'data' => $data
					)))
				);
			} else
			...
			$html .= sprintf(
				...
				<ul id="month-%s" class="contacts-month toggle-container">'
		...
		if( count($periodique_posts) ){
			$html .= sprintf(
				'<li><div class="month-title cont-periodiques toggle-trigger active">%s <span class="nb-items">(%d)</span></div>
				<ul id="periodiques" class="contacts-periodiques toggle-container">'
	...
	*/
	...
	/**
	* Rendu Html des contacts destinés au corps d'un email (newsletter)
	*
	*/
	public static function get_list_for_email($content = '', $options = false){
		...
		if(Agdp_Contact_Post_type::is_diffusion_managed()){
			$term_id = Agdp_Events::get_newsletter_diffusion_term_id();
			self::add_tax_filter(Agdp_Contact::taxonomy_diffusion, $term_id);
		}
...
.agdp-contacts-email .month-title {
	margin-top: 1em;
	margin-bottom: 1em;
	font-size: larger;
	font-weight: bold;
...
.agdp-contacts-email .contact .dates {
	font-size: larger;
	font-weight: bold;
} 
.agdp-contacts-email a-li a-li {
	margin-left: 1em;
	padding-top: 2em;
}
.agdp-contacts-email div.titre, .agdp-contacts-email div.localisation, .agdp-contacts-email div.cont-cities {
	font-weight: bold;
}
.agdp-contacts-email span.cont-nb-places {
	padding-left: 1em;
}
.agdp-contacts-email i {
	font-style: normal;
}
.cont-depart, .cont-arrivee, .cont-date-jour-num {
	font-size: larger;
	font-variant-caps: small-caps;
}
.cont-periodiques, .cont-periodiques .nb-items {
...
'
	...
	*/
	public static function get_list_header($requested_month = false, $options = false){
		...
		if(is_array($selected_terms))
			foreach($selected_terms as $intentionid => $data)
				$filters_summary[] = Agdp_Contact_Post_type::get_intention_label( $intentionid );
		$all_selected_terms[$tax_name] = $selected_terms;
		$taxonomies = array_merge(['cont-intention' => $intentions], $taxonomies);
		
		// Render
		$html = '<div class="agdp-contacts-list-header">'
			...
			. '<p class="agdp-title-link">'
				. '<a href="'. get_page_link( Agdp::get_option('new_contact_page_id')) .'" title="Cliquez ici pour ajouter un nouveau contact"><span class="dashicons dashicons-welcome-add-page"></span></a>'
	...
	/**
	* Rendu Html des contacts d'un mois sous forme de liste
	*/
	public static function get_week_posts_list_html($week, $requested_id = false, $options = false){
		...
		if(is_wp_error( $events)){
			$html = sprintf('<p class="alerte no-events">%s</p>%s', __('Erreur lors de la recherche de contacts.', AGDP_TAG), var_export($events, true));
		}
		elseif($events){
			$html = '';
			if(count($events) === 0){
				$html .= sprintf('<p class="alerte no-events">%s</p>', __('Aucun contact trouvé', AGDP_TAG));
			}
			else {
				...
				if(count($events) == self::$default_posts_per_page){
					$html .= sprintf('<li class="show-more"><h3 class="contact toggle-trigger" ajax="show-more">%s</h3></li>'
	...
	public static function get_list_item_html($post, $requested_id, $options){
		...
		$date_debut = get_post_meta($post->ID, 'cont-date-debut', true);
		...
		$html .= sprintf('<div id="%s%d" class="contact %s %s" contact="%s">'
			, AGDP_ARG_CONTACTID, $post->ID
			, $email_mode ? '' : 'toggle-trigger'
			, $email_mode && ($post->ID == $requested_id) ? 'active' : ''
			, esc_attr( json_encode(['id'=> $post->ID, 'date' => $date_debut]) )
		);
		
		//$cities = Agdp_Contact::get_contact_cities ($post, 'names');
		
		$title = Agdp_Contact::get_post_title( $post, false );
		// $dates = Agdp_Contact::get_contact_dates_text($post->ID);
		$html .= sprintf('<div class="titre">%s</div>', $title);
		
		if( $agdpevents = self::get_contact_agdpevents( $post, false )){
		...
		$value = get_post_meta($post->ID, 'cont-phone', true);
		if($value){
			if( $email_mode && ! $show_phone)
				$value = sprintf('<a href="%s">(masqué)</a>', $url);
			else
				$value = Agdp_Contact::get_phone_html($post->ID);
			$html .= sprintf('<div class="cont-phone">Téléphone : %s</div>',  $value);
		}
		
		if( ! $email_mode && ($agdpevents = self::get_contact_agdpevents( $post, true ))){
 	 ...
 	 */
	public static function get_contact_agdpevents( $contact, $details = true ) {
		
		if( ! $contact
		 || ! Agdp_Contact::is_managed() )
			return '';
		
		$meta_name = 'cont-periodique';
		if( $is_periodique = get_post_meta($contact->ID, $meta_name, true) )
			return '';
		
		//cache
		static $agdpevents_contact_id;
		static $agdpevents;
		if( ! $agdpevents_contact_id || $agdpevents_contact_id !== $contact->ID ){
			$agdpevents_contact_id = $contact->ID;
	...
	public static function download_links(){
		...
		$title = 'Télécharger les contacts au format ICS';
		if( count($_GET) && self::$filters_summary ){
			$title .= ' filtrés (' . self::$filters_summary . ')';
			$data['filters'] = $_GET;
		}
		$html .= Agdp::get_ajax_action_link(false, ['contacts','download_file'], 'download', '', $title, false, $data);
		
		$meta_name = 'download_link';
		foreach(Agdp_Contact::get_all_terms(Agdp_Contact::taxonomy_diffusion) as $term_id => $term){
			$file_format = get_term_meta($term->term_id, $meta_name, true);
			if( $file_format ){
				$data = [
					'file_format' => $file_format,
					'filters' => [ Agdp_Contact::taxonomy_diffusion => $term_id]
				];
				$title = sprintf('Télécharger les contacts pour %s (%s)', $term->name, $file_format);
	...
	public static function on_ajax_action_download_file($data) {
		...
		if( ! $posts)
			return sprintf('Aucun contact à exporter');
