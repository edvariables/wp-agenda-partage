 ...
 *
 * Voir aussi Agdp_Admin_Newsletter
 */
class Agdp_Newsletter {
	...
	public static function init() {
		if ( ! self::$initiated ) {
			...
			self::init_cron(); //SIC : register_activation_hook( 'Agdp_Newsletter', 'init_cron'); ne suffit pas. Pblm de multisite ?
		}
	}
	...
	*/
	public static function filter_anteriority_option($options, $anteriority = false, $default_anteriority = ANTERIORITY_ALL, $newsletter = false){
		if( ! (isset($options['months']) || isset($options['weeks']) || isset($options['days']) || isset($options['hours'])) ){
			if( ! $anteriority ){
				if( ! $newsletter )
					$newsletter = Agdp_Newsletter::get_newsletter();
				if( ! ($anteriority = Agdp_Newsletter::get_anteriority_max($newsletter)) )
	...
	*/
	public static function get_anteriority_date( $newsletter = false, $limit_date = false, $is_posterity = false ){
		if( ! $newsletter )
			$newsletter = Agdp_Newsletter::get_newsletter();
		$anteriority = Agdp_Newsletter::get_anteriority_max($newsletter);
	...
	public static function get_replyto_mail_sender(){
		...
		if( $forum = self::get_forum_of_newsletter() ){
			$emails = Agdp_Forum::get_forum_source_emails($forum);
			if( count($emails) )
				return $emails[0];
		}
		$email = self::get_mail_sender();
		...
		foreach($newsletters as $newsletter_option => $newsletter){
			...
			if( $forum ){
				...
				if( strpos( $html, $input_name ) !== false ){
					$user_forum_subscription = Agdp_Forum::get_user_subscription( $forum, $email );
					$checkboxes = '';/* [select ] === no (no change) */ // sprintf('"%s"', AGDP_WPCF7_RADIO_NO_CHANGE);//option masquée
					$index = 0;
					$selected = '';
					foreach( Agdp_Forum::subscription_roles as $subscribe_code => $label){
						$checkboxes .= sprintf(' "%s|%s"', $label, $subscribe_code);
			...
			if( ! $newsletter ){
				...
				if( ! $newsletter ){
					...
					|| $newsletter->post_type !== self::post_type){
						...
						$newsletter = Agdp::get_option('events_nl_post_id');
					}
				}
			}
		}
	...
	public static function get_forum_of_newsletter($newsletter_id = false){
		...
		if( is_a($newsletter_id, 'WP_Post') ){
			if($newsletter_id->post_type != Agdp_Newsletter::post_type)
				return false;
			$newsletter_id = $newsletter_id->ID;
		}
	...
	public static function get_form_newsletter_field_extension($newsletter_option, $forum = false){
		if( is_a($forum, 'WP_Post') )
			return Agdp_Forum::tag . '-' . $forum->post_name;
		...
		if(is_a($newsletter_option, 'WP_Post')){
			...
			foreach(['events_nl_post_id', 'covoiturages_nl_post_id', 'admin_nl_post_id'] as $option)
				if( $newsletter->ID == Agdp::get_option($option) ){
	...
	public static function get_newsletters(){
		...
		if( $post = get_post(Agdp::get_option($option_id)) )
	...
	public static function create_subscriber_user($email, $user_name, $send_email = true){
		...
		$user = Agdp_User::create_user($email, $user_name, false, false, 'subscriber');
		
		if( ! $user)
			return false;

		if($send_email)
			Agdp_User::send_welcome_email($user, false, false, true);
		return $user;
	}
			...
			foreach( self::get_newsletters() as $newsletter_option => $newsletter){
				...
				if( $forum ){
					...
					if( $user_forum_subscription ) {
						if( isset(Agdp_Forum::subscription_roles[$user_forum_subscription]) )
							$user_forum_subscription_name = Agdp_Forum::subscription_roles[$user_forum_subscription];
						else
			...
			if( $forum ){
				...
				if( empty($inputs[$field_name . AGDP_JS_SKIP_FIELD]) ){
					...
					if($subscription_role !== false){
						if( Agdp_Forum::update_subscription( $email, $subscription_role, $forum))
							$messages['mail_sent_ok'] .= "\r\n".sprintf('L\'adhésion à "%s" est désormais : %s.', $newsletter_name, empty(Agdp_Forum::subscription_roles[$subscription_role]) ? '?' : Agdp_Forum::subscription_roles[$subscription_role]);
					}
				}
			}
	...
	white-space: pre;
			...
			if($success = wp_mail( $to
				...
				, $message
				, $headers, $attachments )){
				if(class_exists('Agdp_Admin', false))
					Agdp_Admin::add_admin_notice(sprintf("La lettre-info a été envoyé à %d destinataire(s).", count($emails)), 'info');
			}
			else{
				if(class_exists('Agdp_Admin', false))
					Agdp_Admin::add_admin_notice(sprintf("L'e-mail n'a pas pu être envoyé"), 'error');
			}
			...
			$success = false;
			if(class_exists('Agdp_Admin', false))
				Agdp_Admin::add_admin_notice(sprintf("Le contenu est vide (aucun élément). Le mail n'a pas été envoyé."), 'warning');
		}
