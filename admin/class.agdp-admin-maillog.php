 ...
 * Dashboard
 *
 * Voir aussi Agdp_Maillog
 */
class Agdp_Admin_Maillog {
	 ...
	 * N'affiche l'éditeur que pour l'trace mail modèle ou si l'option Agdp::agdpmaillog_show_content_editor
	 */
		...
		foreach($agdpmaillogs as $agdpmaillog){
			...
			?><header class="entry-header"><?php 
				if($edit_url)
					...
					$url = Agdp_Maillog::get_post_permalink($agdpmaillog);
				echo sprintf( '<h3 class="entry-title"><a href="%s">%s</a></h3>', $url, Agdp_Maillog::get_agdpmaillog_title($agdpmaillog) );
	...
	/* public static function dashboard_all_agdpmaillogs_cb($post , $widget) {
		...
		foreach($agdpmaillogs as $agdpmaillog){
			echo '<li>';
			edit_post_link( Agdp_Maillog::get_agdpmaillog_title($agdpmaillog), '<h3 class="entry-title">', '</h3>', $agdpmaillog );
			$the_date = get_the_date('', $agdpmaillog);
			$the_modified_date = get_the_modified_date('', $agdpmaillog);
			$html = '';
		...
		echo sprintf('<li><a href="%s">%s...</a></li>', get_home_url( null, 'wp-admin/edit.php?post_type=' . Agdp_Maillog::post_type), __('Tous les traces mail', AGDP_TAG));
		?>
		</ul><?php
	} */
