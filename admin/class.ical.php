<?php
/**
* From https://gist.github.com/Seebz/c00a38d9520e035a6a8c
*/
class iCal
{
	/**
	...
	public function parse($content)
	{
		...
		$this->title = $m ? self::unescape(trim($m[1])) : null;

		// Description
		preg_match('`^X-WR-CALDESC:(.*)$`m', $content, $m);
		$this->description = $m ? self::unescape(trim($m[1])) : null;
...
class iCal_Event
{
	...
	public function parse($content)
	{
		...
		if (preg_match('`^SUMMARY:(.*)$`m', $content, $m))
			$this->summary = iCal::unescape(trim($m[1]));

		// Description
		if (preg_match('`^DESCRIPTION:(.*)$`m', $content, $m))
			$this->description = iCal::unescape(trim($m[1]));
		...
		if (preg_match('`^LOCATION:(.*)$`m', $content, $m))
			$this->location = iCal::unescape(trim($m[1]));
