 ...
 * Dashboard
 *
 * Voir aussi Agdp_Covoiturage
 */
class Agdp_Admin_Covoiturage {
	...
	public static function init_hooks() {
		...
		add_filter( 'manage_' . Agdp_Covoiturage::post_type . '_posts_columns', array( __CLASS__, 'manage_columns' ) );
		add_action( 'manage_' . Agdp_Covoiturage::post_type . '_posts_custom_column', array( __CLASS__, 'manage_custom_columns' ), 10, 2 );
	...
	public static function manage_custom_columns( $column, $post_id ) {
		switch ( $column ) {
			case 'titre' :
				//Evite la confusion avec Agdp_Covoiturage::the_title
			...
			case 'cov_intention' :
				$post = get_post( $post_id );
				echo Agdp_Covoiturage_Post_type::get_intention_label(get_post_meta( $post_id, 'cov-intention', true ));
	 ...
	 * N'affiche l'éditeur que pour le covoiturage modèle ou si l'option Agdp::covoiturage_show_content_editor
	 */
	...
	public static function get_my_covoiturages($num_posts = 5) {
		...
		$sql = "SELECT post.ID, post.post_title, post.post_name, post.post_status, post.post_date, post.post_modified"
			...
			. "\n WHERE post.post_type = '".Agdp_Covoiturage::post_type."'"
		...
		foreach($covoiturages as $covoiturage){
			...
			?><header class="entry-header"><?php 
				if($edit_url)
					...
					$url = Agdp_Covoiturage::get_post_permalink($covoiturage);
				echo sprintf( '<h3 class="entry-title"><a href="%s">%s</a></h3>', $url, Agdp_Covoiturage::get_post_title($covoiturage) );
				...
				if($covoiturage->post_status != 'publish')
					$html .= sprintf('<br><b>%s</b>', Agdp::icon( 'warning',$post_statuses[$covoiturage->post_status])) ;		
	...
	public static function dashboard_all_covoiturages_cb($post , $widget) {
		...
		foreach($covoiturages as $covoiturage){
			echo '<li>';
			edit_post_link( Agdp_Covoiturage::get_post_title($covoiturage), '<h3 class="entry-title">', '</h3>', $covoiturage );
			$the_date = get_the_date('', $covoiturage);
			$the_modified_date = get_the_modified_date('', $covoiturage);
			$html = '';
			...
			$meta_key = 'cov-email';
			$value = Agdp_Covoiturage::get_post_meta($covoiturage, $meta_key, true);
			if($value)
		...
		echo sprintf('<li><a href="%s">%s...</a></li>', get_home_url( null, 'wp-admin/edit.php?post_type=' . Agdp_Covoiturage::post_type), __('Tous les covoiturages', AGDP_TAG));
		?>
		</ul><?php
	}
