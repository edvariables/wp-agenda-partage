 ...
 *
 * Voir aussi Agdp_Mailbox, Agdp_Admin_Mailbox
 */
class Agdp_Admin_Edit_Mailbox extends Agdp_Admin_Edit_Post_Type {

	const post_type = Agdp_Mailbox::post_type; 
	...
	public static function init_hooks() {
		global $pagenow;
		if ( $pagenow === 'post.php' ) {
			add_action( 'add_meta_boxes_' . Agdp_Mailbox::post_type, array( __CLASS__, 'register_mailbox_metaboxes' ), 10, 1 ); //edit
			add_action( 'save_post_' . Agdp_Mailbox::post_type, array(__CLASS__, 'save_post_mailbox_cb'), 10, 3 );
	...
	public static function on_admin_notices_cb(){
		...
		switch($post->post_type){
			...
			case Agdp_Mailbox::post_type:
				...
				//links summary
				$dispatch = Agdp_Mailbox::get_emails_dispatch($post->ID);
					...
					switch($destination['type']){
						case 'page':
							...
							$links[] = sprintf('E-mails vers %s publiés dans <a href="/wp-admin/post.php?post=%s&action=edit">%s</a> (%s).'
								...
								, Agdp_Forum::get_right_label( $destination['rights'])
				...
				if( $links )
					Agdp_Admin::add_admin_notice_now(sprintf('<ul><li>%s</li></ul>', implode('</li><li>', $links))
						, ['type' => 'info']);
				break;
		}
	}
	...
	public static function register_mailbox_metaboxes($post){
		add_meta_box('agdp_mailbox-imap', __('Synchronisation depuis une boîte e-mails', AGDP_TAG), array(__CLASS__, 'metabox_callback'), Agdp_Mailbox::post_type, 'normal', 'high');
		add_meta_box('agdp_mailbox-cron', __('Automate d\'importation', AGDP_TAG), array(__CLASS__, 'metabox_callback'), Agdp_Mailbox::post_type, 'normal', 'high');
	}
	...
	public static function get_metabox_imap_fields(){
		...
		if($test_connexion){
			...
			$result = Agdp_Mailbox::check_connexion($mailbox->ID);
	...
	public static function get_metabox_cron_fields(){
		...
		if($cron_exec){
			...
			$cron_exec_comment = '';
		$cron_exec_comment .= Agdp_Mailbox::get_cron_time_str();
		
		$simulate = ! $cron_exec; //Keep true !
		Agdp_Mailbox::cron_exec( $simulate, $mailbox );
		...
		if( $cron_state = Agdp_Mailbox::get_cron_state() ){
