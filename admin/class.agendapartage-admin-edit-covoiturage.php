 ...
 *
 * Voir aussi AgendaPartage_Covoiturage, AgendaPartage_Admin_Covoiturage
 */
class AgendaPartage_Admin_Edit_Covoiturage extends AgendaPartage_Admin_Edit_Post_Type {
	...
	public static function init_hooks() {
		...
		&& isset($_POST['post_type'])
			&& $_POST['post_type'] === AgendaPartage_Covoiturage::post_type 
		...
		add_action( 'add_meta_boxes_' . AgendaPartage_Covoiturage::post_type, array( __CLASS__, 'register_covoiturage_metaboxes' ), 10, 1 ); //edit
	}
	/****************/
	...
	public static function wp_insert_post_data_cb ($data, $postarr ){
		if($data['post_type'] != AgendaPartage_Covoiturage::post_type)
			return $data;
	...
	public static function create_user_on_save ($data, $postarr){
		...
		if(!$email || !is_email($email)) {
			AgendaPartage_Admin::add_admin_notice("Il manque l'adresse e-mail de l\'organisateur du covoiturage ou elle est incorrecte.", 'error');
			return $data;
		}
		...
		$user = AgendaPartage_User::create_user_for_covoiturage($email, $user_name, $user_login, $user_data, 'subscriber');
		if( is_wp_error($user)) {
	...
	public static function register_metabox_admin(){
		...
		add_meta_box('agdp_covoiturage-admin', $title, array(__CLASS__, 'metabox_callback'), AgendaPartage_Covoiturage::post_type, 'side', 'high');
	}
	...
	public static function on_wp_terms_checklist_args($args, int $post_id){
		if($args['taxonomy'] === AgendaPartage_Covoiturage::taxonomy_diffusion){
